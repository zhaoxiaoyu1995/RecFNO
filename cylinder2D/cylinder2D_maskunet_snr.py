...
# @Author  : zhaoxiaoyu
# @File    : cylinder2D_maskunet_snr.py
import torch
...
from data.dataset import CylinderObserveSNRDataset, CylinderObserveDataset
...
args = parses()
args.exp = 'recon_unet_regre_cylinder_8_snr80'
args.epochs = 300
args.batch_size = 16
print(args)
...
def train():
    ...
    # Build data loader
    train_dataset = CylinderObserveSNRDataset(index=[i for i in range(3500)], snr=True, SNRdB=80)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = CylinderObserveSNRDataset(index=[i for i in range(3500, 4250)], snr=True, SNRdB=80)
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/cylinder2D/logs/ckpt/recon_unet_regre_cylinder_16/best_epoch_298_loss_0.00015960.pth'

    # Define data loader
    test_dataset = CylinderObserveDataset(index=index)
