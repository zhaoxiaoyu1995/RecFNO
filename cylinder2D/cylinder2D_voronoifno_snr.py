...
# @Author  : zhaoxiaoyu
# @File    : cylinder2D_voronoifno_snr.py
import torch
...
from data.dataset import CylinderInterpolSNRDataset, CylinderInterpolDataset
...
args = parses()
args.exp = 'recon_voronoifno_cylinder_8_snr80'
args.epochs = 300
args.batch_size = 16
print(args)
...
def train():
    ...
    net = VoronoiFNO2d(modes1=24, modes2=24, width=32).cuda()

    # Build data loader
    train_dataset = CylinderInterpolSNRDataset(index=[i for i in range(3500)], snr=True, SNRdB=80)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = CylinderInterpolSNRDataset(index=[i for i in range(3500, 4250)], snr=True, SNRdB=80)
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/cylinder2D/logs/ckpt/recon_voronoifno_cylinder_2/best_epoch_299_loss_0.00009405.pth'

    # Define data loader
    test_dataset = CylinderInterpolDataset(index=index)
