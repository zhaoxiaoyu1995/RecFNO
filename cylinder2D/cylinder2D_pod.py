...
args = parses()
args.exp = 'recon_pod_cylinder_32'
args.epochs = 300
args.batch_size = 16
print(args)
...
def train():
    ...
    net = MLP(layers=[32, 64, 64, 64, 25]).cuda()

    # Build data loader
    train_dataset = CylinderPodDataset(pod_index=[i for i in range(3500)], index=[i for i in range(3500)],
                                       n_components=25)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = CylinderPodDataset(pod_index=[i for i in range(3500)], index=[i for i in range(3500, 4250)],
                                     n_components=25)
