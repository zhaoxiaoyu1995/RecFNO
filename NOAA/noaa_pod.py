...
from data.dataset import NOAAPodDataset
...
def train():
    ...
    net = MLP(layers=[256, 64, 64, 64, 50]).cuda()

    # Build data loader
    train_dataset = NOAAPodDataset(pod_index=[i for i in range(1500)], index=[i for i in range(1500)],
                                   n_components=50)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = NOAAPodDataset(pod_index=[i for i in range(1500)], index=[i for i in range(1500, 1700)],
                                 n_components=50)
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/NOAA/logs/ckpt/recon_pod_noaa_256/best_epoch_202_loss_0.00727513.pth'

    # Define data loader
    test_dataset = NOAAPodDataset(pod_index=[i for i in range(1500)], index=index,
                                  n_components=50)
