...
from data.dataset import NOAAObserveDataset
...
args = parses()
args.exp = 'recon_unet_regre_noaa_256'
args.epochs = 300
args.batch_size = 16
print(args)
...
def train():
    ...
    # Build data loader
    train_dataset = NOAAObserveDataset(index=[i for i in range(1500)])
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = NOAAObserveDataset(index=[i for i in range(1500, 1700)])
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/NOAA/logs/ckpt/recon_unet_regre_noaa_256/best_epoch_61_loss_0.00528566.pth'

    # Define data loader
    test_dataset = NOAAObserveDataset(index=index)
