...
def train():
    ...
    net = FNORecon(sensor_num=256, fc_size=(18, 36), out_size=(180, 360), modes1=36, modes2=36, width=32).cuda()

    # Build data loader
    train_dataset = NOAADataset(index=[i for i in range(1500)])
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = NOAADataset(index=[i for i in range(1500, 1700)])
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/NOAA/logs/ckpt/recon_fno_noaa_256/best_epoch_46_loss_0.00741433.pth'

    # Define data loader
    test_dataset = NOAADataset(index=index)
