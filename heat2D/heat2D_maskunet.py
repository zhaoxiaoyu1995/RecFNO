...
args = parses()
args.exp = 'recon_unet_regre_heat_4'
args.epochs = 300
args.batch_size = 8
print(args)
...
def train():
    ...
    # Build data loader
    train_dataset = HeatObserveDataset(index=[i for i in range(4000)])
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = HeatObserveDataset(index=[i for i in range(4000, 5000)])
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/heat2D/logs/ckpt/recon_unet_regre_heat_25/best_epoch_299_loss_0.00011697.pth'

    # Define data loader
    test_dataset = HeatObserveDataset(index=index)
