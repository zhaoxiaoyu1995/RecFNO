...
def train():
    ...
    net = MLP(layers=[4, 64, 64, 64, 25]).cuda()

    # Build data loader
    train_dataset = HeatPodDataset(pod_index=[i for i in range(4000)], index=[i for i in range(4000)],
                                   n_components=25)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = HeatPodDataset(pod_index=[i for i in range(4000)], index=[i for i in range(4000, 5000)],
                                 n_components=25)
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/heat2D/logs/ckpt/recon_pod_heat_36/best_epoch_295_loss_0.00169485.pth'

    # Define data loader
    test_dataset = HeatPodDataset(pod_index=[i for i in range(4000)], index=index,
                                  n_components=25)
