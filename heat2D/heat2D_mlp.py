...
def train():
    ...
    # Build data loader
    train_dataset = HeatDataset(index=[i for i in range(100)])
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = HeatDataset(index=[i for i in range(8000, 9000)])
...
def test():
    ...
    args.snapshot = '/mnt/zhaoxiaoyu/ARE/logs/ckpt/meta/best_epoch_499_loss_0.46464467.pth'

    # Define data loader
    test_dataset = HeatDataset(index=[i for i in range(1024, 2048)])
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=4)

    # Load trained network
    net = MLP(layers=[8, 128, 1280, 4800, 11236]).cuda()
