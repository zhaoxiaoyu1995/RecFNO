...
def train():
    ...
    net = MLP(layers=[36, 128, 1280, 4800, 256 * 256]).cuda()

    # Build data loader
    train_dataset = DarcyDataset(index=[i for i in range(8000)])
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = DarcyDataset(index=[i for i in range(8000, 9000)])
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.val_interval == 0:
            ...
            if epoch % args.plot_freq == 0:
                plot3x1(outputs[-1, 0, :, :].cpu().numpy(), pre[-1, :].reshape(256, 256).cpu().numpy(),
                        file_name=args.fig_path + f'/epoch{epoch}.png')


def test(index=[i for i in range(9000, 10000)]):
    # Path of trained network
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/darcy/logs/ckpt/recon_fno_darcy/best_epoch_100_loss_0.02360215.pth'
    ...
    net = MLP(layers=[16, 128, 1280, 4800, 256 * 256]).cuda()
    ...
    for i, (inputs, outputs) in enumerate(test_loader):
        N, _ = inputs.shape
        inputs, outputs = inputs.cuda(), outputs.cuda()
        # outputs = F.interpolate(outputs, ((256 * 4, 256 * 4)))
        with torch.no_grad():
            pre = net(inputs)
        test_num += N
...
if __name__ == '__main__':
    train()
    # test(index=[i for i in range(9000, 10000)])
