...
from data.dataset import DarcyObserveDataset
...
args = parses()
args.exp = 'recon_fnoreg_daycy_36'
args.epochs = 300
args.batch_size = 16
print(args)
...
def train():
    ...
    net = VoronoiFNO2d(modes1=18, modes2=18, width=32, in_channels=3).cuda()

    # Build data loader
    train_dataset = DarcyObserveDataset(index=[i for i in range(4000)])
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = DarcyObserveDataset(index=[i for i in range(4000, 5000)])
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/darcy/logs/ckpt/recon_fnoreg_daycy_36/best_epoch_299_loss_0.00560629.pth'

    # Define data loader
    test_dataset = DarcyObserveDataset(index=index)
