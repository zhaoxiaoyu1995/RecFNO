...
from data.dataset import DarcyInterpolDataset
...
args = parses()
args.exp = 'recon_voronoifno_daycy_25_display'
args.epochs = 300
args.batch_size = 16
print(args)
...
def train():
    ...
    net = VoronoiFNO2d(modes1=18, modes2=18, width=32, in_channels=4).cuda()

    # Build data loader
    train_dataset = DarcyInterpolDataset(index=[i for i in range(4000)])
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = DarcyInterpolDataset(index=[i for i in range(4000, 5000)])
...
def test(index):
    ...
    args.snapshot = '/home/ubuntu/zhaoxiaoyu/ARE/field_recon/darcy/logs/ckpt/recon_voronoifno_daycy_36/best_epoch_291_loss_0.00638666.pth'

    # Define data loader
    test_dataset = DarcyInterpolDataset(index=index)
